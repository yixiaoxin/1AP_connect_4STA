...
__BSS2_RAM net_if_t fvif_netifs[NX_VIRT_DEV_MAX];
#if (PLF_AIC8800)
PRIVATE_HOST_ARRAY_DECLARE(G1WLAN, net_if_t, NX_VIRT_DEV_MAX, fvif_netifs);
#endif
...
static err_t net_if_output(struct netif *net_if, struct pbuf *p_buf)
{
    ...
    #if CONFIG_WIFI_PING_DEBUG
    net_buf_tx_t *tx_buf = (net_buf_tx_t *)p_buf;
    uint8_t *data = tx_buf->payload;
    if ((data[12] == 0x08) && (data[13] == 0x0) && (data[14] == 0x45) && (data[15] == 0x0)) {
        if ((data[34] == 0x08) && (data[35] == 0x0)) {
            dbg("ICMP(net_out) Request: %d\n", data[41] | (data[40] << 8));
        }
        else if (data[34] == 0x0 && (data[35] == 0x0)) {
            dbg("ICMP(net_out) Reply:   %d\n", data[41] | (data[40] << 8));
        }
    }
    #endif
    ...
    if (netif_is_up(net_if) && netif_send_to_tx_task(net_if, p_buf) == 0)
    {
        status = ERR_OK;
    }
    ...
    if (status != ERR_OK)
    {
        /* pbuf_ref() above added one driver-owned reference.  A failed TX
...
int net_if_input(net_buf_rx_t *buf, net_if_t *net_if, void *addr, uint16_t len, net_buf_free_fn free_fn)
{
    ...
    if ((data[12] == 0x08) && (data[13] == 0x0) && (data[14] == 0x45) && (data[15] == 0x0)) {
        if ((data[34] == 0x08) && (data[35] == 0x0)) {
            dbg("ICMP(net_in) Request: %d\n", data[41] | (data[40] << 8));
        }
        else if (data[34] == 0x0 && (data[35] == 0x0)) {
            dbg("ICMP(net_in) Reply:   %d\n", data[41] | (data[40] << 8));
        }
    }
    #endif
                ...
                const uint8_t *dst_addr, bool *ack)
{
    ...
    if (res == 0)
    {
        // Wait only when the driver accepted the frame and will signal cfm.
        rtos_semaphore_wait(l2_semaphore, -1);
    }
    else
    {
        /* No confirmation callback will arrive after a rejected submission.
         * Release the frame and return promptly so WPA/MLME can retry instead
         * of leaking the pbuf or blocking on the semaphore forever. */
        if (ack) *ack = false;
        pbuf_free(pbuf);
    }
...
int net_set_dns_with_index(uint8_t dns_idx, uint32_t dns_server)
{
    #if LWIP_DNS
    ip_addr_t ip;
    ip_addr_set_ip4_u32(&ip, dns_server);
    dns_setserver(dns_idx, &ip);
    return 0;
    #else
    return -1;
    #endif
...
int net_get_dns_with_index(uint8_t dns_idx, uint32_t *dns_server)
{
    #if LWIP_DNS
    const ip_addr_t *ip;
...
{
    return net_set_dns_with_index(0, dns_server);
}

int net_get_dns(uint32_t *dns_server)
{
    return net_get_dns_with_index(0, dns_server);
}

net_fd_set_ptr_t net_fd_alloc(void)
{
    return (net_fd_set_ptr_t)rtos_malloc(sizeof(fd_set));
}

void net_fd_free(net_fd_set_ptr_t fdp)
{
    rtos_free(fdp);
}

void net_fd_set(int fdn, net_fd_set_ptr_t fdp)
{
    fd_set *p = (fd_set *)fdp;
    FD_SET(fdn, p);
}

void net_fd_clr(int fdn, net_fd_set_ptr_t fdp)
{
    fd_set *p = (fd_set *)fdp;
    FD_CLR(fdn, p);
}

int net_fd_isset(int fdn, net_fd_set_ptr_t fdp)
{
    fd_set *p = (fd_set *)fdp;
    return FD_ISSET(fdn, p);
}

void net_fd_zero(net_fd_set_ptr_t fdp)
{
    fd_set *p = (fd_set *)fdp;
    FD_ZERO(p);
}
...
{
    lwip_arp_data_save();
    memcpy(&backup_fvif_netifs[0], &fvif_netifs[0], sizeof(fvif_netifs));
}

void lwip_data_restore(void)
{
    lwip_arp_data_restore();
    memcpy(&fvif_netifs[0], &backup_fvif_netifs[0], sizeof(fvif_netifs));
}
#endif
